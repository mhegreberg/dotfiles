...
interface_easyname=$(ip route get 1.1.1.1 | grep -Pa '(?<=dev\s)\w+' | awk '{print $5}')
ping=$(ping -c 1 www.google.com | tail -1| awk '{print $4}' | cut -d '/' -f 2 | cut -d '.' -f 1)
netname=$(iwctl station wlan0 show | grep 'Connected network' | awk '{$1=$2=""; print $0}')
